...
const MoveBranchModal: React.FC<MoveBranchModalProps> = ({
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        if (!newTreeId || isNaN(faceNum) || isNaN(posNum)) {
            alert('Please fill out all fields correctly.');
            return;
        }
        onMoveBranch(branch.id, newTreeId, faceNum as 1 | 2 | 3 | 4, posNum);
        onClose();
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative" onClick={e => e.stopPropagation()}>
                <form onSubmit={handleSubmit}>
                    <header className="p-4 border-b flex justify-between items-center">
                        <h2 className="text-xl font-bold text-coral-dark">Move Branch {branch.fragmentId}</h2>
                    ...
                    <div className="p-6 space-y-4">
                         <div>
                            ...
                            <select id="newTree" value={newTreeId} onChange={e => setNewTreeId(e.target.value)} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900">
                         ...
                         <div>
                            ...
                            <input type="number" id="newFace" value={newFace} onChange={e => setNewFace(e.target.value)} required min="1" max="4" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                        </div>
                         <div>
                            <label htmlFor="newPosition" className="block text-sm font-medium text-gray-700">New Position (1-10)</label>
                            <input type="number" id="newPosition" value={newPosition} onChange={e => setNewPosition(e.target.value)} required min="1" max="10" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                        </div>
                    </div>
                    <footer className="p-4 bg-gray-50 rounded-b-2xl flex justify-end gap-2">
                        <button type="button" onClick={onClose} className="bg-gray-200 hover:bg-gray-300 text-coral-dark font-bold py-2 px-4 rounded-lg">Cancel</button>
                        <button type="submit" className="bg-coral-blue text-white font-bold py-2 px-4 rounded-lg">Confirm Move</button>
                    </footer>
                </form>
            </div>
        </div>
        ...
        <form onSubmit={handleAddSubmit} className="p-4 border rounded-lg space-y-4 bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
                <div>
                    ...
                    <select id="addSite" value={addSiteId} onChange={e => { setAddSiteId(e.target.value); setAddTreeId(''); }} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900">
                ...
                <div>
                    ...
                    <select id="addTree" value={addTreeId} onChange={e => setAddTreeId(e.target.value)} required disabled={!addSiteId} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white disabled:bg-gray-100 text-gray-900">
                 ...
                 <div>
                    ...
                    <input type="text" id="addGenus" value={addGenus} onChange={e => setAddGenus(e.target.value)} required placeholder="e.g., Acropora" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                </div>
                <div>
                    <label htmlFor="addSpecies" className="block text-sm font-medium text-gray-700">Species</label>
                    <input type="text" id="addSpecies" value={addSpecies} onChange={e => setAddSpecies(e.target.value)} required placeholder="e.g., palmata" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                </div>
                <div>
                    <label htmlFor="addFace" className="block text-sm font-medium text-gray-700">Face (1-4)</label>
                    <input type="number" id="addFace" value={addFace} onChange={e => setAddFace(e.target.value)} required min="1" max="4" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                </div>
                <div>
                    <label htmlFor="addPosition" className="block text-sm font-medium text-gray-700">Position (1-10)</label>
                    <input type="number" id="addPosition" value={addPosition} onChange={e => setAddPosition(e.target.value)} required min="1" max="10" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                </div>
                 <div className="md:col-span-full flex items-center gap-2 pt-2">
                    <input type="checkbox" id="isHeatTolerant" checked={isHeatTolerant} onChange={e => setIsHeatTolerant(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-coral-blue focus:ring-coral-blue"/>
                    <label htmlFor="isHeatTolerant" className="font-medium text-gray-700">Known heat tolerant colony</label>
                </div>
            </div>
             <div className="flex justify-end pt-2">
                <button type="submit" disabled={!addTreeId} className="bg-coral-blue hover:bg-opacity-90 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200 disabled:bg-gray-400">
                  Add Branch
                </button>
            </div>
        </form>
            ...
            <h3 className="font-semibold text-gray-700 text-lg mb-4">Browse Branches on a Tree</h3>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end p-4 border rounded-lg bg-gray-50 mb-6">
                 <div>
                    ...
                    <select id="browseSite" value={browseSiteId} onChange={e => { setBrowseSiteId(e.target.value); setBrowseTreeId(''); }} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900">
                        <option value="">-- Choose site --</option>
                        {activeSites.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                </div>
                <div>
                    ...
                    <select id="browseTree" value={browseTreeId} onChange={e => setBrowseTreeId(e.target.value)} disabled={!browseSiteId} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white disabled:bg-gray-100 text-gray-900">
             ...
             {browseTreeId && (
                ...
                {branchesOnSelectedTree.length > 0 ? branchesOnSelectedTree.map(([face, faceBranches]) => (
                    <div key={face}>
                        <h4 className="font-medium bg-gray-100 p-2 rounded-t-md">Face {face}</h4>
                            ...
                            {faceBranches.map(branch => {
                                ...
                                return (
                                    <li key={branch.id} className="p-3 flex items-start justify-between gap-2">
                                        <button onClick={() => onSelectBranch(branch.id)} className="w-full flex items-start gap-4 text-left hover:bg-coral-blue/5 rounded-lg p-1">
                                            ...
                                            <div className="flex-grow">
                                                <p className="font-semibold text-coral-dark">{branch.fragmentId}</p>
                                        ...
                                        <button
                                            ...
                                            className="flex-shrink-0 bg-gray-200 hover:bg-gray-300 text-coral-dark font-bold py-1 px-3 rounded-lg text-xs self-center"
                                        >
                                            Move
                                        </button>
                                    </li>
