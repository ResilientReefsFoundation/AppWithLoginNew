...
const DetailItem: React.FC<{ label: string; value?: string | number; children?: React.ReactNode }> = ({ label, value, children }) => (
  <div className="flex justify-between items-center py-1">
    ...
    {value !== undefined && <span className="font-medium text-coral-dark text-right">{value}</span>}
    {children}
  </div>
);

const CoralBranchDisplay: React.FC<CoralBranchDisplayProps> = ({ branch, onOpenPhotoManager, onNavigateToHealthReports, onNavigateToGrowthReports }) => {
    ...
    <div className="space-y-8">
      <div className="p-6 bg-white rounded-lg shadow-lg">
        <div className="flex flex-col md:flex-row justify-between items-start">
            <div>
              <h2 className="text-4xl font-bold text-coral-blue">{branch.fragmentId}</h2>
           ...
           <div className="space-y-3">
            <div className="flex justify-between items-baseline py-1 flex-wrap">
                <div>
                    ...
                    <span className="font-medium text-coral-dark">{new Date(branch.dateAdded).toLocaleDateString()}</span>
                </div>
                <div>
                    <span className="text-gray-600">Age: </span>
                    <span className="font-medium text-coral-dark">{formattedAge}</span>
                </div>
            </div>
            {latestHealthReport && (
             ...
             <DetailItem label="Growth Trend">
              ...
              {branch.isHeatTolerant && (
                <DetailItem label="Trait">
                    <span className="font-medium text-orange-600 bg-orange-100 px-2 py-1 rounded-full text-sm">
                        Known Heat Tolerant
                    </span>
                </DetailItem>
            )}
            ...
            >
              ...
              <div className="mt-4">
                <button 
                  ...
                  className="w-full text-sm bg-gray-200 hover:bg-gray-300 text-coral-dark font-semibold py-2 px-4 rounded-lg transition-colors"
                >
