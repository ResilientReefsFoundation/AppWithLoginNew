...
import type { Reminder, CoralBranch, Site, Tree, ReminderStatus } from '../types';
import { BellIcon, ArrowDownIcon, CloseIcon } from './Icons';
...
const StatCard: React.FC<StatCardProps> = ({ title, value, onClick }) => {
  const content = (
    <>
      ...
      <p className="text-3xl font-bold text-coral-dark">{value}</p>
    </>
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative" onClick={e => e.stopPropagation()}>
            <header className="p-4 border-b flex justify-between items-center">
                <h2 className="text-xl font-bold text-coral-dark">{title}</h2>
            ...
            <footer className="p-4 bg-gray-50 rounded-b-2xl text-right">
                <button onClick={onClose} className="bg-coral-blue hover:bg-opacity-90 text-white font-bold py-2 px-4 rounded-lg">Close</button>
            </footer>
        </div>
    </div>
);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b flex justify-between items-center">
                    <h2 className="text-xl font-bold text-coral-dark">Branches at {site.name} ({status})</h2>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-800">
                        <CloseIcon className="w-6 h-6"/>
                    </button>
                </header>
                    ...
                    {filteredBranches.length > 0 ? (
                        <ul className="divide-y divide-gray-200">
                            {filteredBranches.map(branch => {
                                ...
                                return (
                                    <li key={branch.id}>
                                        <button onClick={() => handleBranchClick(branch.id)} className="w-full flex items-start gap-4 p-3 text-left hover:bg-coral-blue/10 transition-colors rounded-lg">
                                            <img 
...
const DashboardPage: React.FC<{
    ...
    const [siteRemindersModalOpen, setSiteRemindersModalOpen] = React.useState(false);
    ...
    React.useEffect(() => {
        if (selectedSiteId) {
            setSiteRemindersModalOpen(true);
        } else {
            setSiteRemindersModalOpen(false);
        }
    }, [selectedSiteId]);
                ...
                <ul className="mt-2 space-y-3">
                    {items.map(item => (
                        <li key={item.branchId} className="text-sm">
                             <button onClick={() => { onSelectBranch(item.branchId); setSiteRemindersModalOpen(false); setSelectedSiteId(''); }} className="font-semibold text-coral-dark hover:underline text-left">
    ...
    return (
        <div className="space-y-8">
                ...
                <div className="bg-yellow-100 border-l-4 border-yellow-500 rounded-r-lg p-4 shadow">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => setShowReminders(!showReminders)} className="flex items-center gap-1 text-sm font-semibold text-yellow-900">
                           {showReminders ? 'Hide' : 'Show'} Overdue <ArrowDownIcon className={`w-4 h-4 transition-transform ${showReminders ? 'rotate-180' : ''}`}/>
                        </button>
                    </div>
                    {showReminders && (
            ...
            <div>
                <h2 className="text-2xl font-bold text-coral-dark mb-4">Nursery Snapshot</h2>
            ...
            <div>
                <h2 className="text-2xl font-bold text-coral-dark mb-4">Branch Age Distribution</h2>
            ...
            <div>
                <h2 className="text-2xl font-bold text-coral-dark mb-4">Reef Contribution</h2>
            ...
            <div>
                 <h2 className="text-2xl font-bold text-coral-dark mb-4">Site Health Breakdown</h2>
                            ...
                            <div className="flex justify-around items-center text-center text-xs mt-3">
                                {Object.entries(healthBreakdown[site.id]).map(([status, count]) => (
                                    <button
                                        ...
                                        className="flex flex-col items-center p-1 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-coral-blue transition-colors"
             ...
             <div>
                 <h2 className="text-2xl font-bold text-coral-dark mb-4">Species Goal Progress</h2>
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {activeSites.map(site => (
                        <div key={site.id} className="bg-white p-4 rounded-lg shadow">
                                    ...
                                    return (
                                        <div key={species}>
                                            <div className="flex justify-between items-baseline mb-1">
                                                <p className="text-sm font-medium text-coral-dark italic">{species}</p>
                                                <p className="text-sm font-mono text-gray-600">{count}/10</p>
                                            </div>
                                            <div className="w-full bg-orange-100 rounded-full h-2.5">
                                                <div className="bg-coral-green h-2.5 rounded-full" style={{ width: `${progress}%` }}></div>
                                            </div>
                                        </div>
                                    );
                                }) : (
            ...
            {siteRemindersModalOpen && siteReminders && (
                <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={() => setSelectedSiteId('')}>
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative" onClick={e => e.stopPropagation()}>
                        <header className="p-4 border-b flex justify-between items-center">
                            <h2 className="text-xl font-bold text-coral-dark">Tasks for {activeSites.find(s => s.id === selectedSiteId)?.name}</h2>
