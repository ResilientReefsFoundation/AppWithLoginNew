...
const ExperimentButton: React.FC<{ title: string; description: string; onClick?: () => void; disabled?: boolean; }> = ({ title, description, onClick, disabled }) => (
    <button 
        onClick={onClick}
        disabled={disabled}
        className="p-4 bg-gray-50 hover:bg-coral-blue/10 border border-gray-200 rounded-lg text-left transition-colors shadow-sm hover:shadow-md h-full disabled:bg-gray-100 disabled:cursor-not-allowed disabled:shadow-none disabled:opacity-60"
    >
        <h4 className="font-semibold text-coral-dark">{title}</h4>
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b pb-4">
        <h2 className="text-2xl font-bold text-coral-dark mb-2 sm:mb-0">Experiments</h2>
        <button
