...
import type { CoralBranch, Site, Tree, HealthReport, BleachingLevel, Anchor } from '../types';
...
interface MonitoringPageProps {
  ...
  onLogMaintenance: (siteId: string, treeId: string, tasks: string[], notes?: string) => void;
}
...
const MonitoringPage: React.FC<MonitoringPageProps> = ({
  ...
  const handleSaveMaintenance = (e: FormEvent) => {
      e.preventDefault();
      const selectedTasks = Object.entries(maintenanceTasks)
        .filter(([,v]) => v)
        .map(([k]) => MAINTENANCE_TASKS[k as keyof typeof MAINTENANCE_TASKS]);

      if (!maintenanceSiteId || !maintenanceTreeId || selectedTasks.length === 0) {
          alert('Please select a site, a tree, and at least one maintenance task.');
          return;
      }
      
      onLogMaintenance(maintenanceSiteId, maintenanceTreeId, selectedTasks, maintenanceNotes.trim());

      // Reset form and close modal
      setMaintenanceSiteId('');
      setMaintenanceTreeId('');
      setMaintenanceTasks({ snippedTips: false, cleanedTree: false, tightenedTies: false, scrubbedFloats: false });
      setMaintenanceNotes('');
  ...
  const maintenanceTreesForSite = React.useMemo(() => {
   ...
   const atLeastOneTaskSelected = Object.values(maintenanceTasks).some(v => v);
    ...
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg space-y-6">
          ...
          <button
            ...
            className={`whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm ${
              activeTab === 'monitoring'
                ? 'border-coral-blue text-coral-blue'
      ...
      {mode === 'individual' && (
        <>
          <div className="bg-gray-50 p-4 rounded-lg border">
           ...
           <div className="text-center">
                <button
                    ...
                    className="text-coral-blue hover:underline font-semibold"
                >
          ...
          <div className="p-4 border rounded-lg space-y-4 bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select id="siteSelect" value={selectedSite} onChange={e => handleSiteSelection(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900">
              ...
              <div>
                ...
                <select id="treeSelect" value={selectedTree} onChange={e => handleTreeSelection(e.target.value)} disabled={!selectedSite} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white disabled:bg-gray-100 text-gray-900">
                ...
                <div className="border rounded-md max-h-96 overflow-y-auto">
                  <ul className="divide-y divide-gray-200">
                    {branchesOnSelectedTree.map(branch => {
                        ...
                        return (
                            <li key={branch.id}>
                                <button onClick={() => setSelectedBranch(branch)} className="w-full flex items-start gap-4 p-3 text-left hover:bg-coral-blue/10 transition-colors">
          ...
          {selectedBranch && (
            <form onSubmit={handleReportSubmit} className="space-y-6 pt-4 border-t border-coral-green">
              <h3 className="font-semibold text-gray-700 text-lg">New Health Report for: <span className="text-coral-blue">{selectedBranch.fragmentId}</span></h3>
              <div>
              ...
              <div className="flex justify-end gap-2">
                    <button type="button" onClick={() => setSelectedBranch(null)} className="bg-gray-200 hover:bg-gray-300 text-coral-dark font-bold py-2 px-4 rounded-lg">
                        Cancel
                    </button>
                    <button type="submit" className="bg-coral-green text-coral-dark font-bold py-2 px-4 rounded-lg">
                        Submit Report
                    </button>
              </div>
            </form>
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 border rounded-lg grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50">
             <div>
              ...
              <select id="bulkSiteSelect" value={selectedSite} onChange={e => handleSiteSelection(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900">
                <option value="">-- Choose a site --</option>
                {activeSites.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <div>
              ...
              <select id="bulkTreeSelect" value={selectedTree} onChange={e => setSelectedTree(e.target.value)} disabled={!selectedSite} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white disabled:bg-gray-100 text-gray-900">
                <option value="">-- Choose a tree --</option>
                 {treesForSelectedSite.map(t => <option key={t.id} value={t.id}>Tree #{t.number}</option>)}
              </select>
            </div>
               ...
               <div className="border rounded-md max-h-[60vh] overflow-y-auto">
                ...
                {pendingReports.size > 0 && (
                    <div className="flex justify-end mt-4">
                        <button onClick={handleSaveBulkReports} className="bg-coral-green text-coral-dark font-bold py-2 px-6 rounded-lg">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative" onClick={e => e.stopPropagation()}>
            <form onSubmit={handleSaveMaintenance}>
                <header className="p-4 border-b flex justify-between items-center">
                  <h2 className="text-xl font-bold text-coral-dark">Log Maintenance Activity</h2>
                ...
                <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="maintenanceSite" value={maintenanceSiteId} onChange={e => { setMaintenanceSiteId(e.target.value); setMaintenanceTreeId(''); }} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900">
                        ...
                        <div>
                            ...
                            <select id="maintenanceTree" value={maintenanceTreeId} onChange={e => setMaintenanceTreeId(e.target.value)} required disabled={!maintenanceSiteId} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white disabled:bg-gray-100 text-gray-900">
                    ...
                    <div>
                        ...
                        <div className="mt-2 space-y-2">
                           {Object.entries(MAINTENANCE_TASKS).map(([key, label]) => (
                                <label key={key} className="flex items-center gap-2">
                                    <input
                                        type="checkbox"
                                        checked={maintenanceTasks[key as keyof typeof maintenanceTasks]}
                                        onChange={e => setMaintenanceTasks(prev => ({ ...prev, [key]: e.target.checked }))}
                                        className="h-4 w-4 rounded border-gray-300 text-coral-blue focus:ring-coral-blue"
                                    />
                                    <span>{label}</span>
                                </label>
                           ))}
                 ...
                 <footer className="p-4 bg-gray-50 rounded-b-2xl flex justify-end gap-2">
                     <button
                      ...
                      <button
                        type="submit"
                        disabled={!maintenanceSiteId || !maintenanceTreeId || !atLeastOneTaskSelected}
                        className="bg-coral-green hover:bg-opacity-90 text-coral-dark font-bold py-2 px-4 rounded-lg transition-colors duration-200 disabled:bg-gray-300"
                      >
