      ...
      <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-full max-h-[90vh] flex flex-col">
          <header className="p-4 border-b flex justify-between items-center">
            <h2 className="text-xl font-bold text-coral-dark">
              {isSelectMode ? `Selected (${selectedPhotoIds.length})` : "Photo Gallery"}
            </h2>
            {isSelectMode ? (
              <button onClick={handleCancelSelection} className="text-sm font-semibold text-coral-blue hover:text-opacity-80">Cancel</button>
            ) : (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {photos.map((photo, index) => (
                      <div key={photo.id} className="relative group aspect-square cursor-pointer" onClick={() => handlePhotoClick(index)}>
                          ...
                          <div className={`absolute inset-0 bg-black ring-4 rounded-lg transition-all duration-300 ${selectedPhotoIds.includes(photo.id) ? 'bg-opacity-40 ring-coral-blue' : 'bg-opacity-0 ring-transparent'}`}></div>
                          ...
                          {isSelectMode && (
                              <div className={`absolute top-1.5 right-1.5 p-0.5 rounded-full transition-all duration-300 ${selectedPhotoIds.includes(photo.id) ? 'bg-coral-blue scale-100' : 'bg-white/50 scale-90 opacity-70 group-hover:opacity-100'}`}>
            ...
            {isSelectMode ? (
              <div className="w-full flex justify-between items-center gap-4">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed"
                  >
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-2">
                  <button
                    ...
                    className="w-full sm:w-auto bg-gray-200 hover:bg-gray-300 text-coral-dark font-bold py-2 px-4 rounded-lg transition-colors duration-200"
                  >
                    Select
                  </button>
                  <button
                    onClick={handleClose}
                    className="w-full sm:w-auto bg-coral-blue hover:bg-opacity-90 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200"
                  >
                    Done
                  </button>
              </div>
             ...
             {!isSelectMode && (
                <div 
                    ...
                    onDrop={handleDrop}
                    className={`relative border-2 border-dashed rounded-lg p-6 text-center transition-colors duration-300 ${isDragging ? 'border-coral-blue bg-blue-50' : 'border-gray-300 bg-gray-50'}`}
                >
                    <UploadIcon className="mx-auto h-10 w-10 text-gray-400"/>
                    <p className="mt-2 text-sm text-gray-600">Drag & drop to upload</p>
                    <p className="text-xs text-gray-500">or</p>
                    <label htmlFor="file-upload" className="font-medium text-coral-blue hover:text-opacity-80 cursor-pointer">
                        browse files
                    </label>
