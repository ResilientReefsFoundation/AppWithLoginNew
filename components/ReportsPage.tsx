    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative">
        <header className="p-4 border-b flex justify-between items-center">
          <h2 className="text-xl font-bold text-coral-dark">{report.title} Report</h2>
                ...
                if (config[statKey] && value > 0) {
                    return (
                        <div className="flex justify-between" key={statKey}>
                          ...
                          <span className="font-semibold text-coral-dark text-lg">{value}</span>
                        </div>
                    );
                }
                return null;
    ...
    <>
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg space-y-8">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b pb-4">
          <h2 className="text-2xl font-bold text-coral-dark mb-2 sm:mb-0">Reports</h2>
          <button
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
              <ChartBarIcon className="w-6 h-6 text-coral-blue" />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {MOCK_REPORTS_DATA.map(report => (
              <button
                ...
                className="p-4 bg-gray-50 hover:bg-coral-blue/10 border border-gray-200 rounded-lg text-left transition-colors shadow-sm hover:shadow-md"
              >
                <h4 className="font-semibold text-coral-dark">{report.title}</h4>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="border-t pt-8">
            <div className="flex items-center gap-2 mb-4">
                <CalendarDaysIcon className="w-6 h-6 text-coral-green" />
            ...
            <div className="p-4 bg-gray-50 border rounded-lg space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" id="startDate" value={startDate} onChange={e => setStartDate(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                </div>
                <div>
                  <label htmlFor="endDate" className="block text-sm font-medium text-gray-700">End Date</label>
                  <input type="date" id="endDate" value={endDate} onChange={e => setEndDate(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-white text-gray-900"/>
                </div>
              </div>
