    ...
    return (
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg space-y-6">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b pb-4">
                <h2 className="text-2xl font-bold text-coral-dark mb-2 sm:mb-0">Rules Management</h2>
                <button
            ...
            <form onSubmit={handleAddRuleSubmit} className="p-4 border rounded-lg space-y-4 bg-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div>
                  ...
                  <select id="ruleTarget" value={newRuleTarget} onChange={(e) => setNewRuleTarget(e.target.value as RuleTarget)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-coral-blue focus:ring-coral-blue sm:text-sm p-2 bg-white text-gray-900">
                ...
                <div>
                  ...
                  <input type="number" id="ruleInterval" value={newRuleInterval} onChange={(e) => setNewRuleInterval(e.target.value)} min="1" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-coral-blue focus:ring-coral-blue sm:text-sm p-2 bg-white text-gray-900" />
                </div>
                <div>
                  <label htmlFor="ruleCheckType" className="block text-sm font-medium text-gray-700">Check Type</label>
                  <select id="ruleCheckType" value={newRuleCheckType} onChange={(e) => setNewRuleCheckType(e.target.value as CheckType)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-coral-blue focus:ring-coral-blue sm:text-sm p-2 bg-white text-gray-900">
