      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative">
        <form onSubmit={handleSubmit}>
            <header className="p-4 border-b flex justify-between items-center">
              <h2 className="text-xl font-bold text-coral-dark">Edit Site</h2>
            ...
            <div className="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" id="siteNameEdit" value={name} onChange={e => setName(e.target.value)} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2"/>
                </div>
                <div>
            ...
            <footer className="p-4 bg-gray-50 rounded-b-2xl flex justify-end gap-2">
                ...
                <button type="submit" className="bg-coral-green text-coral-dark font-bold py-2 px-4 rounded-lg">Save Changes</button>
            </footer>
        </form>
      </div>
    </div>
        ...
        <form onSubmit={handleAddSite} className="p-4 border rounded-lg space-y-4 bg-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <div>
              ...
              <input type="text" id="siteName" value={newSiteName} onChange={e => setNewSiteName(e.target.value)} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2"/>
            </div>
            <div className="flex items-end gap-4">
              ...
              <div className="flex-grow">
                 ...
                 <input type="file" id="sitePhoto" onChange={handleFileChange} required accept="image/*" className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-coral-blue/10 file:text-coral-blue hover:file:bg-coral-blue/20"/>
              </div>
            </div>
          </div>
          <div className="flex justify-end">
            <button type="submit" className="bg-coral-blue hover:bg-opacity-90 text-white font-bold py-2 px-4 rounded-lg">
              Add Site
            </button>
          </div>
        </form>
